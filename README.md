...
For the MIMIC-III database, the diagnosis and procedure codes are extracted from the 'DIAGNOSES_ICD.csv' and 'CPTEVENTS.csv'. For the eICU database, the diagnosis and procedure codes are extracted from the 'diagnosis.csv' and 'treatment.csv'. 
<table>
	<tr>
...
We evaluate the performance of our method against deep imputation methods. For the imputation task, we replace the fully connected layer in GRU-D with a regression layer. For the prediction task, we replace the linear decoder in MTSIT with a Softmax output layer; we feed the output of Conditional GAN, STING, MBGAN, and SA-EDGAN into GRU to estimate ICU mortality risk probabilities. 
